...
const addAExerciseShowAllExercises = ref(false)
const addAExerciseAllExercises = ref([]);
const addAExerciseAllExercisesChecked = ref([]);
...
async function addAExercise() {
  try {
    ...
    if (addAExerciseAllExercisesChecked.value.length) {
      ...
      const res = await api.post(`/api/v1/logs/multiple`, logs);
      const json = await res.json();
...
function clearDataAndDismissAddAExerciseModal() {
  addAExerciseAllExercisesChecked.value = []
  const modal = bootstrap.Modal.getOrCreateInstance(document.getElementById(`add-a-exercise`));
  modal.hide();
  window.setTimeout(() => { addAExerciseShowAllExercises.value = false }, 300);
}
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div v-if="addAExerciseAllExercisesChecked.length" class="d-flex justify-content-center flex-wrap gap-1 mb-3">
            <div class="badge bg-secondary text-white" v-for="ex in addAExerciseAllExercisesChecked">{{ ex.name }}
            </div>

            <!-- debug -->
            <!-- <pre class="alert alert-danger">{{ addAExerciseAllExercisesChecked }}</pre>
            <pre class="alert alert-success">{{ currentSessionDetails?.logs }}</pre> -->
          </div>

          <!-- Disable/Enable selection -->
          <div class="form-check form-switch mb-3">
            <input @click="addAExerciseAllExercisesChecked.length ? addAExerciseAllExercisesChecked = [] : null"
          ...
          <!-- multi exercises -->
          <span v-if="!addAExerciseShowAllExercises">
            <!-- label container -->
            ...
            <div class="rounded border border-1 p-2"
              ...
              <span v-for="category in addAExerciseAllExercises" :key="category.id">
                ...
                <div class="form-check" v-for="exercise in category.exercises" :key="exercise.id">
                  <input class="form-check-input" type="checkbox" :value="exercise" :id="exercise.name"
                    :disabled="currentSessionDetails?.logs?.map((cur => cur.exercise_id)).includes(exercise.id)"
                    v-model="addAExerciseAllExercisesChecked">
                  <label class="form-check-label"
                    :class="{ 'strike': currentSessionDetails?.logs?.map((cur => cur.exercise_id)).includes(exercise.id) }"
                    :for="exercise.name">{{ exercise.name }} {{ exercise.id }}</label>
                </div>
          ...
          <span v-else>
            ...
            <div class="mb-3">
              ...
              <select id="session-details-select-exercise" class="form-control form-select form-select-sm"
                ...
                required>
                <option v-for="exercise in chooseExercises" :value="exercise.id" :key="`key-${exercise.id}`"
                  :disabled="currentSessionDetails?.logs?.map((cur => cur.exercise_id)).includes(exercise.id)">
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-success"
            :disabled="addAExerciseLoading || (addAExerciseShowAllExercises && (!chooseExerciseId || !addAExerciseAllExercisesChecked)) || (!addAExerciseShowAllExercises && addAExerciseAllExercisesChecked.length === 0)">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body text-center">
          ...
          <div>
            ...
            <div @click="$refs.video.click()" class="alert alert-primary d-flex flex-column gap-1 m-0 p-0 py-3 my-1"
